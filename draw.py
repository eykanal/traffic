...
class App(object):
    def __init__(self, master):
        ...
        self.road_width = self.lane_width * sum(self.lane_count) + (
            1 * (sum(self.lane_count) - 1)
        )  # total road width, +1 to account for lines between lanes
        self.c_v_mid = self.canvas_size[1] // 2
        self.road_bottom = self.c_v_mid - (
            (self.road_width // 2) + 1
        )  # +1 for lines between lanes
        self.spawn_locs = []
        ...
        for i, loc_bottom in enumerate(lane_locs[:-1]):
            ...
            self.c.create_rectangle(
                ...
                loc_bottom,
                self.canvas_size[0],
                loc_bottom + self.lane_width + 1,
                fill="dark gray",
                width=0,
                tags="road %s" % (lane_tags[i]),
            )
            if i > 0:
                self.c.create_line(
                    0,
    ...
    def _find_lane_locs(self):
        ...
        # extra +1 is for the top border... each lane
        # has a bottom border, the very top lane has a
        # top border as well
        bottom = (self.canvas_size[1] // 2) - (
            (width * count) + 1
        ) // 2
    ...
    def _get_lane_tags(self, car_id):
        ...
        l = set(self.c.find_overlapping(coo[0], coo[1], coo[2], coo[3])) - set(
            self.c.find_withtag("car")
        )
    ...
    def move(self):
        for i, car in list(self.cars.items()):
            ...
            if car["speed"] != car["happy_speed"] and (car["mdir"] in (0, math.pi)):
                ...
                if self.c.gettags(next_lane):
                    ...
                    coo_new.append(
                        coo[0] - self.car_size
                        if car["dir"] == -1
                        else coo[0] - self.car_size * 1.5
                    )
                    coo_new.append(
                        coo[1] + self.lane_width * (-1 if car["dir"] == 1 else 1)
                    )
                    coo_new.append(
                        coo[2] + self.car_size
                        if car["dir"] == 1
                        else coo[2] + self.car_size * 1.5
                    )
                    coo_new.append(
                        coo[3] + self.lane_width * (-1 if car["dir"] == 1 else 1)
                    )
                    cars_in_the_way = set(
                        self.c.find_overlapping(
                            coo_new[0], coo_new[1], coo_new[2], coo_new[3]
                        )
                    ) - set(self.c.find_withtag("road"))
            ...
            self.c.move(
                i,
                round(car["speed"] * math.cos(car["mdir"]), 4),
                round(car["speed"] * math.sin(car["mdir"]), 4),
            )
    ...
    def spawn(self):
        ...
        for l in self.spawn_locs:
            ...
            if (
                len(self.c.find_overlapping(bound[0], bound[1], bound[2], bound[3]))
                == 1
            ):
                if rand.gammavariate(1, 1) > self.spawn_freq:
                    car_id = self.c.create_rectangle(
                        l["loc"][0] - 5,
                        l["loc"][1] - 5,
                        l["loc"][0] + 5,
                        l["loc"][1] + 5,
                        fill=self.car_colors.get("normal"),
                        activefill="yellow",
                        tags="car",
                    )
